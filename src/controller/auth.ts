...
const register = async (req: IRegisterRequest, res: IRegisterResponse) => {
  ...
  // Validate client provided fullname of valid length
  if (req.body.fullname.length < 3 || req.body.fullname.length > 26) {
    ServerGlobal.getInstance().logger.error(
      `<register>: Failed to register since provided invalid fullname with email ${req.body.email}`
    );
  ...
  try {
    ...
    const matchingUser: Readonly<Omit<mongoose.Document, "id">> | null =
      await UserDB.findOne({ email: req.body.email }, { id: 0 });
...
const login = async (req: ILoginRequest, res: ILoginResponse) => {
  ...
  try {
    const userByEmail: Omit<IUserDocument, "email"> | null =
